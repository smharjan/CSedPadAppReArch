...
public class ExpectAnswer
{
	...
	{
		Element[] answerNodes = XMLFilesManager.GetXMLChildrenElements(node,
				"Text");
		if (answerNodes.length > 0)
		{
			acceptedAnswer = answerNodes[0].getTextContent();
			// get the list of accepted answer; we could possibly have
			// BadWithFeedback also
			...
			for (int i = 0; i < variants.getLength(); i++)
			{
				...
				goodAnswerVariants[i] = variant.getElementsByTagName("Text")
						.item(0).getTextContent();
				goodFeedbackVariants[i] = variant
						.getElementsByTagName("Feedback").item(0)
						.getTextContent();
			}
			if (node.getElementsByTagName("Wrong").getLength() > 0)
				wrongAnswer = node.getElementsByTagName("Wrong").item(0)
						.getTextContent();
			;
		}
		else
	...
	public boolean HasAllTheRequiredWords(String text, Boolean negationCheck)
	{
		...
		if (NLPManager.getInstance().MatchRegularExpression(semText,
				acceptedAnswer, negationCheck))
			return true;

		// check the answer variants if the initial answer requirements are not
		// met
		for (int i = 0; i < goodAnswerVariants.length; i++)
			if (NLPManager.getInstance().MatchRegularExpression(semText,
					goodAnswerVariants[i], negationCheck))
			{
				matchedVariant = i;
				return true;
			}
	...
	public boolean HasForbbidenWords(String text)
	{
		...
		// Vasile: wrongAnswer should be replaced with checking forbidden words;
		// anyhow this function is not called at all as of Oct 31, 2012
		if (NLPManager.getInstance().MatchRegularExpression(semText,
				wrongAnswer, true))
			return true;
	...
	{
		if (matchedVariant < 0)
			return XMLFilesManager.getInstance().GetSomeFeedback(
					"PositiveFeedback");
		else
			return goodFeedbackVariants[matchedVariant];
	}
