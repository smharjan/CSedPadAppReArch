package memphis.deeptutor.gui.commands;
...
public class DTCommands
{
	...
	public String getCommands(Components c)
	{
		String command = "<DTCommands>";

		if (c.getProgress().contains("/"))
		{
			command += "<command type=\"changeCurrentTaskTitle\">";
			command += "<title>Current Task: " + c.getProgress() + "</title>";
			command += "</command>";
		}
		...
		if (c.getQuestion() != null)
		{
			...
			{
				command += "<image>" + "<src>"
						+ c.getQuestion().getImage().getSource() + "</src>"
						+ "<height>" + c.getQuestion().getImage().getHeight()
						+ "</height>" + "<width>"
						+ c.getQuestion().getImage().getWidth() + "</width>"
						+ "</image>";
			}

			if (c.getMultimedia() != null)
			{
				command += "<multimedia>" + "<src>"
						+ c.getMultimedia().getSource() + "</src>" + "<type>"
						+ c.getMultimedia().getType() + "</type>" + "<height>"
						+ c.getMultimedia().getHeight() + "</height>"
		...
		if (c.getMultimedia() != null)
		{
			command += "<command type=\"changeMultimedia\">" + "<multimedia>"
		...
		// the clear history command must be put before setting up the tutor
		// response
		...
		if (c.studentID != null)
		{
			command += "<command type=\"setStudentID\">" + c.studentID
					+ "</command>";
		}
		...
		if (c.getResponse() != null)
		{
			...
			for (int i = 0; i < c.getResponse().getResponseCount(); i++)
				command += "<response>" + c.getResponse().getResponseText(i)
						+ "</response>";
			command += "</command>";
		}
		...
		if (c.getAvatar() != null)
		{
			command += "<command type=\"changeAvatar\">" + "<source>"
					+ c.getAvatar().getSource() + "</source>" + "</command>";
		}

		if (c.getNotice() != null)
		{
			command += "<command type=\"changeInformation\">" + "<information>"
					+ c.getNotice().getNotice() + "</information>"
					+ "</command>";
		}
	...
	public static void main(String args[])
	{
		...
		av.setSource("http://localhost:8080/CSedPadAppReArch/includes/Avatar.swf");
